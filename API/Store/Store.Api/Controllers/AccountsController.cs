...
namespace Store.Api.Controllers
{
    ...
    public class AccountsController : ControllerBase
    {
        ...
        public async Task<ActionResult<UserToken>> GetTokenByCode([FromBody] String code,string jpushId,bool isClientServer=false)
        {
            try
            {
                if (string.IsNullOrEmpty(jpushId))
                    return BadRequest("JPush id can't be empty.");
                ...
                if (_userFreshTokenRepo.IsExist(userInfo.Email))
                {
                    ...
                    user.IsClientServer = isClientServer;
                    _userFreshTokenRepo.Update(userInfo, newJwtToken.RefreshToken);
                    return Ok(newJwtToken);
                }
                else
                {
                    var user = new ApplicationUser { UserName = userName, Email = userName, JPushId=jpushId,IsClientServer=isClientServer };
